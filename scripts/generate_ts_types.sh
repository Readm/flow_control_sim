...
OUTPUT_FILE="$PROJECT_ROOT/web_dev/src/types/api.ts"
...
# 进入 web_dev 目录
cd "$PROJECT_ROOT/web_dev"

# 检查 openapi-typescript 是否安装
if [ ! -f "node_modules/.bin/openapi-typescript" ]; then
    echo "错误: openapi-typescript 未安装"
    echo "请运行: cd web_dev && npm install --save-dev openapi-typescript"
    exit 1
fi

# 生成类型定义
# 注意：openapi-typescript 需要读取 yaml，这里我们指向主仓库的 yaml
npx openapi-typescript "$OPENAPI_FILE" --output "src/types/api.ts"
