...
### 2.4 轻量级同步优化 (Single-Waiter Optimization)
针对 1:1 的端口连接特征，我们废弃了昂贵的 `sync.Cond` 广播机制：
- **信道通知 (Channel Signaling)**：使用容量为 1 的 `chan struct{}` 替代 `sync.Cond`。发送方通过非阻塞 `select` 发送信号，接收方直接阻塞在信道读取上。这消除了条件变量内部维护等待队列的锁开销。
- **无锁快速路径 (Lock-free Fast Path)**：在 `UpdateReady` 等高频操作中，引入原子操作（CAS）。如果状态更新是顺序的，系统将完全绕过互斥锁，直接通过原子变量推进状态。
- **性能结果**：优化后，同步层在 CPU 消耗中的占比从 **13% 降至 3%**，互斥锁竞争延迟几乎归零。
