...
var metadataLockEnabled atomic.Bool

// EnableMetadataLocking toggles metadata read/write locking.
// Visualization模式下传 true，headless 模式下传 false 以避免多余开销。
func EnableMetadataLocking(enabled bool) {
	metadataLockEnabled.Store(enabled)
}
...
func (p *Packet) SetMetadata(key, value string) {
	...
	enabled := metadataLockEnabled.Load()
	if enabled {
		p.metadataMu.Lock()
...
// ClonePacketMetadata safely clones the metadata map from a packet when locking is enabled.
func ClonePacketMetadata(pkt *Packet) map[string]string {
	if pkt == nil {
		return nil
	}
	enabled := metadataLockEnabled.Load()
	if enabled {
		pkt.metadataMu.RLock()
		defer pkt.metadataMu.RUnlock()
	}
	return CloneMetadata(pkt.Metadata)
}
