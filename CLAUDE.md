...
- **Go Test Timeout**: 所有 `go test` 命令使用 `-timeout=20s` 标志
  - 示例: `go test -timeout=20s ./...`
  - 原因: 防止测试无限期挂起
...
## Shared Guidelines
- **Schema-First Workflow**: 为了防止 Go/TS 类型与 OpenAPI 定义不同步：
  - **CI Enforcement**: CI 会检查 generated code 是否过时，不一致则失败。
  - **Git Hooks**: 推荐运行 `git config core.hooksPath .githooks`，在 commit 时自动更新生成代码。
  - **Manual**: 修改 `openapi.yaml` 后，必须运行 `scripts/generate_go_types.sh` 和 `scripts/generate_ts_types.sh`。
