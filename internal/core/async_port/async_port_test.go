...
func TestChainThreeFlows(t *testing.T) {
	...
	port01 := NewPort(8) // Flow0 -> Flow1
	port12 := NewPort(8) // Flow1 -> Flow2
...
func TestChainWithBackpressure(t *testing.T) {
	...
	port01 := NewPort(2) // Small buffer to trigger backpressure
	port12 := NewPort(2)
...
func TestChainParallelComputation(t *testing.T) {
	...
	port01 := NewPort(100) // Large buffer, no backpressure
	port12 := NewPort(100)
